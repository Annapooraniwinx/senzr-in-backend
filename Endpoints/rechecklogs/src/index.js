...
// 4️⃣ Parse logs for valid employees, starting after lastProcessedRow
function extractLogs(data, validMap, tenantId, lastProcessedRow = -1) {
  ...
  for (let [rowIndex, row] of data.entries()) {
    if (rowIndex <= lastProcessedRow) continue; // ⬅ Skip already processed rows
    ...
    if (row[1] === "Employee Code:-") {
      ...
      if (!logCount.has(empCode)) logCount.set(empCode, 0);
    }
...
export default function registerEndpoint(router, { services, database }) {
  ...
  router.post("/", upload.single("file"), async (req, res) => {
    ...
    try {
      ...
      const { tenantId, importID, fileId, lastProcessedRow = -1 } = req.body;
      const { file } = req;
      ...
      console.log(`▶️ Resuming from row: ${lastProcessedRow + 1}`);
