export default (router, { services }) => {
  router.post("/re-attendance", async (req, res) => {
    ...
    console.log(
      "🚀 jeson Starting attendance processing for",
...
async function deleteExistingAttendance(
  ...
  if (existingRecords.length > 0) {
    const keys = existingRecords.map((record) => record.id);
    await attendanceService.deleteMany(keys);
    console.log(`🗑️ Deleted ${keys.length} existing attendance records`);
  }
}
...
async function fetchLogs(
  ...
  return await logsService.readByQuery({
    ...
    sort: ["timeStamp"], // Sort by timestamp at database level
    limit: -1,
  });
}
...
function calculateAttendanceForDate(
  ...
  if (todayShifts.length > 0) {
    const shiftId = parseInt(todayShifts[0]); // Use first shift
  ...
  // Process logs: Find earliest and latest timestamp
  let firstIn = null;
  let lastOut = null;
  let mode = null;
  ...
  // Calculate working time - simple calculation, no midnight crossover
  const workingMins =
  ...
  // Determine action and status based on logs
  let action;
  let status;
  if (dayLogs.length === 0) {
    action = "notPunchedIn";
...
function calculateAttendanceDeduction(
  ...
  if (lateHours > 0) {
    // Assume no entryTimeLimit
    const type = policy.workingHoursType || "lop";
    const leave = "CL"; // Default, since wrkHoursLeave removed
    ...
    if (type === "lop") {
      ...
      penaltyAmount += 0; // No amount field, default to 0
